<?php 
    ...
    if(isset($_GET['del'])) {
      ...
      if(mysqli_num_rows($sql_se) > 0) {
        $icon         = @mysqli_result_me($sql_se,0,'icon');
        $duongdantin  = @mysqli_result_me($sql_se,0,'duongdantin');
        ...
        {
          while($row = mysqli_fetch_assoc($sql_img))
          {
            $p_name   = $row['p_name'];
    ...
    if($id > 0) {
      ...
      if(mysqli_num_rows($baiviet) == 0) {
        LOCATION_js($full_url);
        exit();
      }
      $baiviet = mysqli_fetch_assoc($baiviet);
