...
<div class="pagewrap page_conten_page">
  ...
  <!--  -->
  <?php
      $danhmuc    = LAY_danhmuc($slug_step,"","`id_parent` = 0");
      foreach ($danhmuc as $dm) {
      $lay_all_kx = LAYDANHSACH_idkietxuat($dm['id'], $slug_step);
      $baiviet    = LAY_baiviet($slug_step,24,"`id_parent` IN ($lay_all_kx)");
      $cehck_seoname = str_replace("/","",$dm['seo_name']);
      $cehck_seoname = DB_que("SELECT `id` FROM `#_danhmuc` WHERE `seo_name` = '$cehck_seoname' LIMIT 1");
      if(!DB_num($cehck_seoname)) continue;
  ...
  <div class="box_more">
    ...
    <div class="more_pro">
      <h3><a <?=full_href($dm) ?>><?=$glo_lang['xem_them'] ?><i class="fa fa-angle-double-down"></i></a></h3>
    </div>
  </div>
  <?php } ?>
  <!-- bang gia -->
