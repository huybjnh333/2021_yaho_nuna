    ...
    <div class="pagewrap">
        <ul>
            <li><i class="fa fa-home"></i><a
                        href="<?= $full_url ?>"><?= $glo_lang['trang_chu'] ?></a><?= GET_bre($arr_running['id_parent'], $slug_step, $full_url, $lang, $thongtin_step, $slug_table, '<i class="fa fa-angle-right"></i>') ?>
            </li>
            <div class="clr"></div>
        </ul>
    </div>
    ...
    <div class="padding_pagewrap <?= $arr_running['link_video'] == "" ? "full_pro_img_main" : "" ?>">
        <div id="pro_img_main">
            <div id="bridal_images">
                <a href='<?= checkImage($fullpath, $arr_running['icon'], $arr_running['duongdantin']) ?>'
                   class='cloud-zoom ' id='zoom1' rel="position: 'inside' , showTitle: false, adjustX:0, adjustY:0"><img
                    ...
                    foreach ($list_hinhcon as $rows) {
                        ...
                        <div class="item">
                            <li><a href='<?= checkImage($fullpath, $rows['icon'], $rows['duongdantin']) ?>'
                                   ...
                                   rel="useZoom: 'zoom1', smallImage: '<?= checkImage($fullpath, $rows['icon'], $rows['duongdantin']) ?>'"><?= full_img($rows) ?></a>
                            </li>
                        </div>
                    <?php } ?>
                </div>
        ...
        if ($arr_running['link_video'] != "") {
            ...
            <div class="product-video">
                <iframe src="<?= $arr_running['link_video'] ?>" title="Camilla" allowtransparency="true" frameborder="0"
                        scrolling="no" class="wistia_embed" name="wistia_embed" allowfullscreen="allowfullscreen"
                        mozallowfullscreen="mozallowfullscreen" webkitallowfullscreen="webkitallowfullscreen"
                        oallowfullscreen="oallowfullscreen" msallowfullscreen="msallowfullscreen" width="384"
                        height="600" style="width: 384px; height: 576px;"></iframe>
            </div>
        <?php } ?>
                ...
                <form action="<?= $full_url . "/gio-hang/" ?>" method="post">
                    <ul>
                                ...
                                <div class="chon_size">
                                    ...
                                    <ul>
                                        ...
                                        for ($i = 0; $i < count(${'gia_tri_' . $k . '_vi'}); $i++) {
                                            ?>
                                            <li>
                                                <input class="js_tinhnang_getval" type="radio" name="tinhnang_<?= $k ?>"
                                                       value="<?= ${'gia_tri_' . $k . '_vi'}[$i] ?>"
                                                       id="id_tn_<?= $k ?>_<?= $val[$i] ?>">
                        ...
                        <h2>
                            ...
                            <span class="p_yeuthich"><a class="cur check_click_<?= $arr_running['id'] ?>"
                                                        onclick="yeu_thich(<?= $arr_running['id'] ?>, this)"><i
                    ...
                    <div class="item">
                        <ul>
                            ...
                            <div class="view_more_pro">
                                <div class="clolum_id p_yeuthich"><a class="cur check_click_<?= $rows['id'] ?>"
                                                                     onclick="yeu_thich(<?= $rows['id'] ?>, this)"><i
                                                class="fa fa-heart-o" aria-hidden="true"></i></a></div>
                                <div class="clolum_id"><a <?= full_href($rows) ?>><i class="fa fa-eye"
                                                                                     aria-hidden="true"></i></a></div>
                                <div class="clolum_id"><a href="<?= $full_url . "/gio-hang/?id=" . $rows['id'] ?>"><i
