<?php
    ...
    if (count($check_slug) > 0) {
        ...
        if($slug_table == "baiviet" || $slug_table == "danhmuc") {
            $arr_running = DB_fet_rd("*", "`#_$slug_table`", "`id` = '" . $slug_id . "'", "", 1);
        }
        else {
    ...
    // if($motty == "" && $thongtin['is_intro'] == 1){
    //     if(empty($_SESSION['intro'])){
    //         $_SESSION['intro'] = 1;
    //         LOCATION_js($full_url."/intro");
    //         exit();
    //     }
    // }
    // if($motty == "intro"){
    //     include _source."intro.php";
    //     exit();
    // }
