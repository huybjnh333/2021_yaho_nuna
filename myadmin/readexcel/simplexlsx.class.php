	...
	private function _unzip( $filename, $is_data = false ) {
		...
		if ( $is_data ) {
			...
			if ( ! is_readable( $filename ) ) {
				$this->error( 'Không tìm thấy tệp ' . $filename );
		...
		*/
		if ( ( $pcd = strrpos( $vZ, "\x50\x4b\x05\x06" ) ) === false ) {
			$this->error( 'Định dạng lưu trữ không xác định!' );
...
//		$this->styles = array();
	...
	public function getEntryXML( $name ) {
		...
		$this->error( 'Không tìm thấy mục nhập XML: ' . $name );
	...
	public function getEntryData( $name ) {
		...
		$this->error( 'Mục nhập không tìm thấy: '.$name );
	...
	public function worksheet( $worksheet_id ) {
		...
		$this->error( 'Bảng tính ' . $worksheet_id . ' không tìm thấy.' );
	...
	private function _columnIndex( $cell = 'A1' ) {
		...
		$this->error( 'Chỉ số không hợp lệ ' . $cell );
