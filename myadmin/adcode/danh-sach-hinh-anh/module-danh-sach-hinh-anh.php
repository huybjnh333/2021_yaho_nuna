...
if (!DB_num($loaibanner)) {
    LOCATION_js("index.php");
    exit();
}
$loaibanner = DB_arr($loaibanner, 1);
...
if (isset($_GET['them-moi']) || (isset($_GET['edit']) && is_numeric($_GET['edit']))) {
    ...
    $sql = DB_que("SELECT * FROM `$table` WHERE `id_parent` = '$id_parent' $mo ORDER BY $odby  `catasort` DESC,`id` DESC LIMIT $pzz,$numview");
    ...
    <section class="content-header">
        <h1><?php if (isset($_SESSION['admin'])){ ?><a class="js_okkk" style="cursor: pointer;" onclick="LOAD_sort()">[SORT] <?php } ?></a><?= $loaibanner['tenbaiviet_vi'] ?>
        </h1>
        <ol class="breadcrumb">
                                ...
                                <div class="dv-hd-locsds">
                                    <div class="form-group">
                                        <select name='docid' class="js_trangthai_js form-control"
                                                onchange='SEARCH_jsstep()' style="min-width: 150px">
                        ...
                        <div class="box-body table-responsive no-padding table-danhsach-cont">
                            <table class="table table-hover table-danhsach">
                                ...
                                <tr>
                                    ...
                                    <th>Tiêu đề</th>
                                    <?php
                                        if($thongtin_banner['is_hinhanh'] == 1){
                                    ?>
                                ...
                                foreach ($sql as $rows) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="name">
                                                <a href="<?= $url_page ?>&id-parent=<?= $id_parent ?>&edit=<?= $ida ?>"
                                                   title="Cập nhật"><?= $tenbaiviet_vi ?></a>
                                                <p class="p_lang_en"><?= $tenbaiviet_en ?></p>
                                                <?php if ($check_video == 1) { ?>
                                                    <p class="p_lang_en" style="color: red;"><?= $video ?></p>
                                                <?php } ?>
                                            </div>
                                            <?php
                                            if ($id_parent == 34) {
