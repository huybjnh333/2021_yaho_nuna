                            ...
                            <table class="table table-hover table-danhsach">
                                ...
                                foreach ($sql as $rows) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="name">
                                                <input type="text" name="tenbaiviet_vi<?= $id ?>" class="cls_emty_name"
                                                       value="<?= $tenbaiviet_vi ?>" placeholder="Link nguồn...">
                                            </div>
                                            <div class="name" id="en">
                                                <input type="text" name="lien_ket<?= $id ?>" class="cls_emty_name"
                                                       value="<?= $lien_ket ?>" placeholder="Link đến...">
                                            </div>
                                            <p style="font-size: 10px; color: rgba(41, 127, 202, 0.55); margin: 0; margin-top: 2px;">
                                                Thự hiện: <?= number_format($thuc_hien) ?> - Lần
                                                cuối: <?= date("d-m-Y H:i:s", $lan_cuoi) ?></p>
                                        </td>
                                        <td class="text-center">
                                            <div id="cus" class="cus_menu">
                                                <label><input type="checkbox" name="showhi<?= $id ?>"
                                                              value="1" <?= (($showhi == 1) ? "checked='checked'" : "") ?>
                                                              class="minimal minimal_click" colum="showhi"
                                                              idcol="<?= $id ?>" table="<?= $table ?>"
                                                              value='1'></label>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <input name='xoa_gr_arr_<?= $id ?>' type='checkbox'
