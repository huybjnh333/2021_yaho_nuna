...
if (!empty($_POST)) {
    ...
    $_POST['val_min'] = @is_numeric($val_min) ? $val_min : 0;
    $_POST['val_max'] = @is_numeric($val_max) ? $val_max : 0;
    $_POST['loai_hienthi'] = isset($loai_hienthi) ? $loai_hienthi : 1;
    $_POST['duongdantin'] = @$duongdantin;
    $_POST['tim_kiem'] = @is_numeric($tim_kiem) ? $tim_kiem : 0;
...
<section class="content-header">
    <h1><?php if (isset($_SESSION['admin'])) { ?><a style="cursor: pointer;" onclick="AUTO_dich(this)">[NGÔN
            NGỮ]</a> <?php } ?>Danh sách tính năng</h1>
    <ol class="breadcrumb">
                            ...
                            foreach ($arr_lang as $rows) {
                                ...
                                <div class="tab-pane <?= $count_lang == 1 ? "active" : "" ?>"
                                    ...
                                    <div class="form-group">
                                        <label>Tên tính năng (<?= $lang ?>)</label>
                ...
                <div class="box p10">
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                <label class="mr-20 hover" style="font-weight: 500;">
                                    <input type="radio" name="loai_hienthi"
                                           value="1" <?= !isset($loai_hienthi) || $loai_hienthi == 1 ? 'checked="checked"' : "" ?>>
                                    Kiểu chọn
                                </label>
                                <label class="mr-20 hover" style="font-weight: 500;">
                                    <input type="radio" name="loai_hienthi"
                                           value="0" <?= isset($loai_hienthi) && $loai_hienthi == 0 ? 'checked="checked"' : "" ?>>
                                    Kiểu nhập
                                </label>
                            </div>
                        </div>
                    ...
                    if ($is_mamau) { ?>
                        <div class="tab-pane" id="tab_4" style="display: none">
                            <div class="form-group">
                                <label>Mã màu <input type="color" onchange="$('#ma_mau').val($(this).val())"
                                                     style="padding: 0; width: 20px; height: 20px; border-radius: 100px; border: none; display: inline-block; background: none; margin-left: 5px; cursor: pointer;"
                                                     value="<?= !empty($ma_mau) ? SHOW_text($ma_mau) : '' ?>"></label>
                                <input type="text" class="form-control"
