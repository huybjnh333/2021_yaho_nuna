<?php 
	...
	if (!empty($_POST['apos']) && $_POST['apos'] == "send_img_bs64ajax") {
		...
		echo '<img src="../' . $duongdantin . "/" . $hinhanh . '" class="img"> <a class="cur jaax_sort_data" dataid="'.$id.'"  onclick="remove_images_js(this, ' . $id . ')">X</a>';
		exit();
	}
	// 
	// xoa anh img_ajjax_
	...
	if(isset($_POST['admindangnhap']))
	{
		...
		$sqlu = DB_que("SELECT `id`,`matkhau`,`keypass`, `phanquyen` FROM `#_members` WHERE `showhi` = 1 AND `tentruycap`='".addslashes($_POST['username'])."' AND `phanquyen` <> 0 LIMIT 1");
		if(@DB_num($sqlu))
		{
			$sqlu 			= DB_arr($sqlu, 1);
