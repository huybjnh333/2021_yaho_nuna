...
if (isset($_GET['them-moi']) || (isset($_GET['edit']) && is_numeric($_GET['edit']))) {
    ...
    $mo .= " AND `loaihienthi` = $loaihienthi";
    ...
    $thongtin_step = LAY_step($step_id, 1);

    ?>
    <section class="content-header">
        <h1><?php if (isset($_SESSION['admin'])){ ?><a class="js_okkk" style="cursor: pointer;" onclick="LOAD_sort()">[SORT] <?php } ?></a><?= $thongtin_step['tenbaiviet_vi'] ?>
        </h1>
        <ol class="breadcrumb">
                    ...
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title box-title-td pull-right">
                                ...
                                <a href="?module=main-module&action=danh-sach-bai-viet&step=<?= $step_id ?>&id_step=<?= $thongtin_step['step'] ?>"
                        ...
                        <div class="box-body table-responsive no-padding table-danhsach-cont">
                            <table class="table table-hover table-danhsach">
                                ...
                                <tr>
                                    ...
                                    <th>Tiêu đề</th>
                                    <?php if ($thongtin['bvct_hinhanh'] == 1) { ?>
                                        <th class="w100 text-center">Hình ảnh</th>
                                    <?php }
                                    if ($thongtin['bvct_file'] == 1) { ?>
                                        <th class="w100 text-center">File</th>
                                    <?php } ?>
                                ...
                                foreach ($sql as $rows) {
                                    ...
                                    <tr>
                                        <td class="text-center">
                                            ...
                                            <input type="text" class="js_sorty text-center" value="<?= $catasort ?>"
                                                   onchange="UPDATE_colum(this, '<?= $ida ?>', 'catasort','<?= $table ?> ')">
                                        </td>

                                        <td>
                                            <div class="name">
                                                <a href="<?= $url_page ?>&id-parent=<?= $id_parent . "&step=" . $step_id ?>&edit=<?= $ida ?>&loaihienthi=<?= $loaihienthi ?>"
                                        ...
                                        if ($thongtin['bvct_file'] == 1) { ?>
                                            <td class="text-center">
                                                <a style="display: block; max-width: 400px; overflow: hidden; }"
                                                   href="<?= $dowload_text ?>"
                                                   target="_blank"><?= $rows['dowload_text'] ?></a>
                                                <a style="display: block; max-width: 400px; overflow: hidden; }"
                                                   href="<?= $fullpath . "/datafiles/files/" . $dowload ?>"
                                                   download><?= $rows['dowload'] ?></a>
                                            </td>
                                        <?php } ?>
                                        <td class="text-center">
