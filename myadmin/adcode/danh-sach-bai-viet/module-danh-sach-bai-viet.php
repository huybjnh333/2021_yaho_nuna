...
if (!DB_num($thongtin_step)) {
    LOCATION_js($fullpath_admin);
    exit();
}
...
if (isset($_GET['upload']) && (isset($_GET['edit']) && is_numeric($_GET['edit']))) {
        ...
        for ($i = 0; $i < $_REQUEST['maxvalu']; $i++) {
            ...
            if (isset($_POST["xoa_gr_arr_$i"])) {
                ...
                if (DB_num($sql_se) > 0) {
                    ...
                    DB_que("DELETE FROM `#_baiviet_select_nhomgia` WHERE `id_baiviet` = '" . $idofme . "'", "#_baiviet_select_nhomgia");
                ...
                if ($hinhanh != false) {
                    ...
                    add_img_admin($thongtin_step['opt1'], @$_POST['anh_sp_' . $i], $idofme, $sql_thongtin, $duongdantin, $hinhanh, 'icon');
                }
                //end
            ...
            foreach ($_FILES['is_muti_file']['name'] as $name => $value) {
                ...
                if (DB_num($sql)) {
                    ...
                    if (move_uploaded_file($_FILES['is_muti_file']['tmp_name'][$name], $uploaddir . $img_real_name)) {
                        ...
                        add_img_admin($thongtin_step['opt1'], @$size_img, 0, @array(), $duongdantin, $img_real_name, 'icon');
                        ...
                        // }
                        // TAO_anhthumb($uploaddir.$img_real_name,$uploaddir."thumbnew_".$img_real_name, 300, 300);
                    }
                } else {
                    break;
                }
    ...
    <section class="content-header">
        <h1><?php if (isset($_SESSION['admin'])) { ?><a onclick="LOAD_sort()"
                                                        class="cur load_okkk">[SORT]</a> <?php } ?><?= GETNAME_step($step) ?>
        </h1>
        <ol class="breadcrumb">
                ...
                <section class="col-lg-12">
                    <div class="box">
                        ...
                        if (isset($_SESSION['admin'])) {
                            ...
                            <div class="dv-nhom-sppp">
                                <label class="mr-20 checkbox-mini">
                                    <input type="checkbox" name="is_giamuti"
                                           class="minimal minimal_click" <?= isset($thongtin['is_giamuti']) && $thongtin['is_giamuti'] == 1 ? 'checked="checked"' : '' ?>
                                           colum="is_giamuti" idcol="1" table="#_seo" value="1"> Giá muti sp
                                </label>
                            </div>
                        <?php } ?>
                        <div class="dv-show-setting">
                            ...
                            <?= admin_input_setting(69) ?>
                            <?= admin_input_setting(70) ?>
                            <?= admin_input_setting(71) ?>

                            <?= admin_input_setting_textarea(62) ?>
                            <?= admin_input_setting(12) ?>
                            <?= admin_input_setting(63) ?>
                            <?= admin_input_setting(68) ?>
                            <?= admin_input_setting(57) ?>
                            <div class="clr"></div>
                        </div>
                        <!--  -->
                    </div>
