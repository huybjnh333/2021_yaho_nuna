...
function update_anh_sort() {
    setTimeout(function () {
        $(".jaax_sort_data").each(function (index) {
            $(this).attr("sort", index);
            $(this).addClass("jaax_sort_data_run");
        });
        update_anh_sort_run();
    }, 100);
}

function update_anh_sort_run() {
    if ($(".jaax_sort_data_run").length == 0) return false;
    var id = $(".jaax_sort_data_run").eq(0).attr("dataid");
    var sort = $(".jaax_sort_data_run").eq(0).attr("sort");
    $.ajax({
        type: "POST",
        url: "index.php",
        data: {
        ...
        success: function (data) {
            $(".jaax_sort_data_run").eq(0).removeClass("jaax_sort_data_run");
            update_anh_sort_run();
            update_anh_sort();
        }
    });
    ...
    document.querySelector('#upload_mutile_tindang').onchange = function () {
        ...
        [].forEach.call(this.files, function (file) {
            ...
            reader.onloadend = function () {
                $('.dv-anh-js-load').append('<div class="dv_hinhanh_con dv-anh-chl dvgrimgs-js js_ajax_run_upload"><img src="' + reader.result + '" class="img" isname="' + file.name + '"> <a class="cur" onclick="remove_images_js(this, 0)"><img src="images/x_icon.svg" alt=""></a></div>');
            }
...
function dang_anh_ajax() {
    $(".js_ajax_run_upload").append('<img src="images/loadernew.gif" class="load"  style="top: 50%; left: 50%; position: absolute; transform: translate(-50%,-50%); width: 40px; height: 40px;">');
    dang_anh_runrun();
}
