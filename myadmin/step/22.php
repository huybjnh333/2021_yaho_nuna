    ...
    <div class="tab-content">
        ...
        foreach ($arr_lang as $rows) {
            ...
            <div class="tab-pane <?= $count_lang == 1 ? "active" : "" ?>" id="tab_<?= $count_lang ?>">
                ...
                <div class="form-group">
                    <label>Nội dung dưới (<?= $lang ?>)</label>
                    <textarea id="thongtin_<?= $lang ?>" name="thongtin_<?= $lang ?>" class="form-control paEditor">
            <?= !empty(${"thongtin_" . $lang}) ? SHOW_text(${"thongtin_" . $lang}) : '' ?></textarea>
                    <!--<input type="text" class="form-control " name="thongtin_<?= $lang ?>"
                           id="thongtin_<?= $lang ?>"
                           value="<?= !empty(${"thongtin_" . $lang}) ? SHOW_text(${"thongtin_" . $lang}) : '' ?>">-->
                </div>
...
<div class="box p10">
    ...
    <div class="form-group">
        ...
        <?php
        $array_kht = array(1 => "Dự đoán ngày sinh", 2 => "Xem tuổi của bé");
        ?>
        <select name="p2" class="form-control">
            <option value="">Chọn kiểu hiển thị</option>
            <?php foreach($array_kht as $key => $value) { ?>
                <option value="<?= $key ?>" <?= @LAY_selected($key, $p2) ?>><?=$value?></option>
            <?php } ?>
        </select>
    </div>
