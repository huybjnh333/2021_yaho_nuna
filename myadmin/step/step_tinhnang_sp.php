<style type="text/css">
    ...
    .dv-tinhnang-con label input[type="file"] {
        margin-top: 10px;
        border: none;
        padding: 0;
    }
...
<div class="box p10">
    <div class="form-group">
        <label>Thuộc tính đặt hàng <a
                    data-tooltip="Tối đa 3 thuộc tính (Ví dụ: Màu, dung lượng, tính năng)"> </a></label>
    </div>
    <!--  -->
    <?php
    if ($id > 0) {
            ...
            <div class="dv-lbtinhnang flex">
                <div class="dv-tinhnang-con flex">
                    ...
                    foreach ($tinhnang_arr as $val2) {
                        ...
                        ?>
                        <label>
                            <input type="checkbox" class="js_check_arr_dh" dataname="<?= $val2['tenbaiviet_vi'] ?>"
                                   datas="<?= $kjs ?>" name="tinhnang_dh[]"
                                   value="<?= $value['id'] . '_' . $val2['id'] ?>" <?= !empty($list_arr_dh) && in_array($value['id'] . '_' . $val2['id'], @$list_arr_dh) ? 'checked="checked"' : "" ?>>
                            <?= $val2['tenbaiviet_vi'] ?>
                            <?php if ($value['tieu_bieu'] == 1) { ?>
                                <?php if (@$list_hinh[$value['id'] . '_' . $val2['id']]['icon'] != "") { ?>
                                    <div class="clr"></div>
                                    <img src="../<?= $list_hinh[$value['id'] . '_' . $val2['id']]['duongdantin'] ?>/<?= $list_hinh[$value['id'] . '_' . $val2['id']]['icon'] ?>"
                                         style="width: 60px; max-height: 50px; float: left; height: auto; margin-top: 10px;">
                                    <input type="file" name="upload_file_<?= $value['id'] . '_' . $val2['id'] ?>"
                                           style="width: calc(100% - 65px); float: right;">
                                <?php } else { ?>
                                    <input type="file" name="upload_file_<?= $value['id'] . '_' . $val2['id'] ?>">
                                <?php } ?>
                            <?php } ?>
...
<div class="box p10">
    ...
    <div class="dv-pro-thuoctinh form-group">
        ...
        <a class="cur addtt"
           onclick="them_thuoc_tinh()" <?= @$thuoc_tinh_2_vi != "" && $thuoc_tinh_3_vi != "" ? 'style="display: none;"' : '' ?>
           style="display: none">Thêm thuộc tính khác</a>
