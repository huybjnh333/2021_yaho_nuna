...
//								            		$check_dis3 = 'disabled="disabled"';
	...
	function UPLOAD_image($file, $folder, $newname = ''){
		if(isset($_FILES[$file]) && !$_FILES[$file]['error']){
			...
			if($_FILES[$file]['type'] == "image/jpeg" || $_FILES[$file]['type'] == "image/png" || $_FILES[$file]['type'] == "image/gif" || $_FILES[$file]['type'] == "image/x-icon" || $_FILES[$file]['type'] == "image/vnd.microsoft.icon" || $_FILES[$file]['type'] == "image/svg" || $_FILES[$file]['type'] == "image/svg+xml" || $_FILES[$file]['type'] == "image/webp")
			{
	...
	function TAO_anhthumb_new($filenameImage, $filenameThumb, $thumb_width, $thumb_height) {
	    $ext      = explode(".", $filenameImage);
	    ...
	    if($ext   == "bmp") {
	        $image    = @imagecreatefrombmp($filenameImage);
	    }
	    ...
	    switch ($ext) {
	        ...
	        case 'gif':
	        {
	            imagegif($thumb,$filename,1);break;
	        }
	        case 'bmp':
	        {
	            imagejpeg($thumb,$filename,1);break;
	        }
	        default:
	        {
	...
	function LAYTEXT_rieng($id) {
		$seo_name = DB_fet_rd("*", "#_seo_name", "`showhi` = 1 AND `id` = '$id'", "", 1);
	...
	function SHOW_text($text, $glo_get_qc = ''){
	    ...
	    if(defined("MOTTY")) {
	    	...
	    	preg_match_all("/\[QCGG](.+?)\[\/QCGG]/", $text, $check_id_qc);
	    	foreach ($check_id_qc[1] as $valqc) {
	    		if(is_numeric($valqc) && !empty($glo_get_qc[$valqc]['ma_quang_cao'])){
		    		$text = preg_replace("/\[QCGG](.*)\[\/QCGG]/i", '<div class="dv-quang-cao-gg dv-quang-cao-gg-0">'.$glo_get_qc[$valqc]['ma_quang_cao'].'</div>', $text);
		    	}
	    	}
	...
	function THEM_seoname($id, $seo_name, $bang, $step, $loai = 0, $i = 0){
		...
		if(strstr($seo_name,"http://") != '' || strstr($seo_name,"tel:") != '' || strstr($seo_name,"mailto:") != '' || strstr($seo_name,"https://") != '' || strstr($seo_name,"/") != '') {
			$data['slug'] 		= $seo_name_goc;
		}

		if(DB_num($id_slug)){
			$id_slug 			= DB_arr($id_slug, 1);
			$id_slug 			= $id_slug["id"];
	...
	function THONGKE_online() {
		$sql = DB_que("SELECT * FROM `#_counter` WHERE `id` = 1 LIMIT 1");
		$sql = DB_arr($sql, 1);
		return $sql['coonter'];
	}
	...
	function LAY_email($type){
		$list_email 	= '';
		$sql 			= DB_que("SELECT * FROM `#_email_config` WHERE `type` IN (".$type.") AND `showhi` = 1");
		$sql 			= DB_arr($sql);
		foreach ($sql as $rim) {
			if(empty($list_email))
	...
	function GUI_email($to_email,$to_name,$subject,$domain,$body, $thongtin, $admin = ""){
	    ...
	    if($thongtin['em_ip'] == "smtp.gmail.com") {
	    	$mail->SMTPSecure = 'tls';
	    	$mail->Port = 587;
	    }
	    ...
	    $frommail         = $thongtin['em_taikhoan']; //"info@".$domain;
    ...
    function so_luong_theo_dmy($day,$month,$year) {
    	...
    	if(DB_num($check) > 0){
        	$sql_a = DB_que("SELECT SUM(count) AS `so_nguoi` FROM `#_count_date` WHERE `day` = '$day' AND `month` = '$month' AND `year` = '$year' LIMIT 1");
        	$sql_a = DB_arr($sql_a, 1);
            return $sql_a["so_nguoi"];
        } else {
        	return 0;
        }
    }
	...
	function COPPY_row($table, $id, $step = 0){
		...
		if($table == "baiviet") {
			...
			$list_data_tn = DB_arr($list_data_tn);
			foreach ($list_data_tn as $rs) {
				$data  = array();
				// print_r($rs);
			...
			$list_data_tn = DB_que("SELECT * FROM `#_baiviet_select_nhomgia` WHERE `id_baiviet` = '$id_bv'");
			$list_data_tn = DB_arr($list_data_tn);
			foreach ($list_data_tn as $rs) {
				$data  = array();
				// print_r($rs);
				...
				ACTION_db($data, "#_baiviet_select_nhomgia", "add", NULL);
			}
			//
		}
		//
	...
	function check_chon_module_danhmuc($row, $class_ul, $class_li, $class_a, $tb_danhmuc, $img_1 = "", $img_2 = ""){
		global $full_url, $lang;
		if($row['step'] != 0) {
			if($row['kieu_hien_thi'] == 2) { //baiviet
				...
				foreach ($tb_listbv as $val) {
					...
					$return    .= '<li class="'.$class_li.'"><a class="'.$class_a.'" href="'.GET_link($full_url, $val['seo_name']).$them.'" icons="&rsaquo;">'.$val['tenbaiviet_'.$lang].'</a></li>';
				}
				return $return != "" ? "<ul class='".$class_ul."'>".$img_1.$return."</ul>" : $return;
				// end
			}
				...
				//
				return $list_dm_sel != "" ? "<ul class='".$class_ul."'>".$img_1.$list_dm_sel."</ul>" : $list_dm_sel;
			}
		}
	}
		...
		foreach ($tb_menu as $row_0) {
			...
			foreach ($tb_menu as $row_1) {
				...
				if($row_1['kieu_chon'] == 0) {
					...
					$list_mb_2  .= check_chon_module_danhmuc($row_1, $class_ul, $class_li, $class_a, $tb_danhmuc,$icon,$icon_hover);
				}
				//end cap 2
	...
	function GET_gia($gia, $giakm, $dvt = '', $lienhe = '', $class_gia = '', $class_km = '', $name_gia = '', $name_km  = '', $ismuti = 0, $idbv = 0){
		...
		if($ismuti == 1){
			foreach ($baiviet_gia as $rgia) {
				$check = LAY_gia("`id_baiviet` = '$idbv' AND `id_nhomgia` = '".$rgia['id']."' AND `id_val` <> 0", 1);
				if(is_array($check) && count($check)){
					//
					$check    = reset($check);
					$gia      = $check['id_val'];
					$text_gia = $gia != 0 ? number_format($gia).' <span class="dvt">'.$dvt.' - '.$rgia['tenbaiviet_'.$lang].'</span>' : $lienhe;
					return array("gia" => $gia, "km" => 0, "pt" => 0, "text_gia" => $name_gia." <span class='".$class_gia."'>".$text_gia."</span>", "text_km" => "");
					break;
				}
			}
			return array("gia" => 0, "km" => 0, "pt" => 0, "text_gia" => $name_gia." <span class='".$class_gia."'>".$lienhe."</span>", "text_km" => "");
		}
		else {
			$km 		= 0;
			if($giakm != 0) {
	...
	function LAY_dulieusn($where = ""){
		...
		$thuoctinhchung = DB_fet_rd("*", "`#_du_lieu_sn`", "`showhi` = 1 $where", "`catasort` ASC, `id` DESC", 0, "id");
	...
	function LAY_baiviet($step, $limit = 0, $where = "", $orderby = '', $col = '',$showsql = ""){
		...
		$baiviet  = DB_fet_rd("*", "`#_baiviet`", "`step` IN ($step) $where", "$orderby", $limit, $col,"",$showsql);
		return $baiviet;
	}
	function LAY_baiviet_chitiet($id = 0, $loaihienthi = 0 ,$catasort = "`catasort` DESC, `id` DESC" , $limit = ""){
		$baiviet  = DB_fet_rd("*", "`#_baiviet_chitiet`", "`id_parent` = '$id' AND `loaihienthi` = $loaihienthi ", "$catasort",$limit);
		return $baiviet;
	}
	...
	function LAY_bv_gia($step, $limit = 0, $where = "",$order_by = ""){
		if($limit == 0) $limit = "";
		if($where != "") $where = " AND $where";
		$baiviet  = DB_fet("*","`#_baiviet_nhomgia`","`showhi` = 1 AND `step` IN ($step) $where ","$order_by `catasort` ASC, `id` DESC",$limit,"arr", 1);
		return $baiviet;
	}
	function LAY_tinhnang($step, $limit = 0, $where = "",$order_by = ""){
		if($limit == 0) $limit = "";
		if($where != "") $where = " AND $where";
		$baiviet  = DB_fet_rd("*","`#_baiviet_tinhnang`","`showhi` = 1 AND `step` IN ($step) $where ","$order_by `catasort` ASC, `id` DESC",$limit, "id");
		return $baiviet;
	}
	function LAY_gia($where = "", $limit = 0, $order_by = ""){
		if($limit == 0) $limit = "";
		if($where != "") $where = " AND $where";

		$baiviet  = DB_fet_rd("*","`#_baiviet_select_nhomgia`","`showhi` = 1 $where ","$order_by  `id` DESC",$limit, "id");
		return $baiviet;
	}
	...
	function LAY_danhmuc($step, $limit = 0, $where = "", $catasort = '', $col = '',$showsql = ""){
		...
		if($col == '') {
			$danhmuc  = DB_fet_rd("*", "`#_danhmuc`", "`step` IN ($step) $where ", "$catasort , `id` DESC", $limit,"","",$showsql);
		}
		else {
			$danhmuc  = DB_fet_rd("*", "`#_danhmuc`", "`step` IN ($step) $where ", "$catasort , `id` DESC", $limit, $col,"",$showsql);
		}
		return $danhmuc;
	}
	...
	function RETURN_text_lienhe_download($name, $value){
		if($value == "") return;
		return '<tr><td colspan="3" style="width:160px; font-size: 13px">'.LOC_char(base64_decode($name)).'</td><td colspan="4" width="400"><span style="display:block; padding-left:5px; font-size: 13px">'.$value.'</span></td></tr>';
	}
	...
	function ADMIN_show_img ($duongdan, $icon) {
		...
		if($ext == "svg"){
			...
			if(@getimagesize($duongdan."/thumb_".$icon)){
	...
	function check_gia_sql($id_sp, $id_bvsl, $dongia){
		...
		if($thongtin['is_giamuti'] == 1) {
			...
			$bvtinhnang   = DB_fet("*","`#_baiviet_select_nhomgia`","`id_baiviet` = '".$id_sp."' AND `id_nhomgia` = '".trim($id_bvsl)."' ","","","arr");
			// print_r($bvtinhnang);
			if(count($bvtinhnang)) {
				return $bvtinhnang[0]['id_val'];
			}
			return 0;
		}
		else {
			...
			$id_bvsl = trim($id_bvsl);
		    $id_bvsl = explode(",",$id_bvsl);
		    sort($id_bvsl);
	...
	function full_src($rows, $thumb = 'thumb_',$col = "icon")
		{
			global $fullpath;
			if($rows['icon'] != '') {
				return $fullpath."/".$rows['duongdantin']."/".$thumb.$rows[$col];
	...
	function full_src_lazy($rows, $thumb = 'thumb_',$col = "icon")
	{
		global $fullpath,$lang;
		$no_image = $fullpath."/images/no_image_trang.png";
		return 'src="'.$no_image.'" data-srcset="'.$fullpath.'/'.$rows['duongdantin'].'/'.$thumb.$rows[$col].' 2x,'.$fullpath.'/'.$rows['duongdantin'].'/'.$thumb.$rows[$col].' 1x" data-src="'.$fullpath.'/'.$rows['duongdantin'].'/'.$thumb.$rows[$col].'" alt="'.$rows['tenbaiviet_'.$lang].'"';
	}
	...
	function full_href($rows, $uri = "", $col = "seo_name", $targ = "blank") {
		global $full_url;
		$link = $rows[$col];
		$blank = "";
		if(strstr($link,"http://") != '' ||
  		...
  		if(strstr($link,"http://") != '' ||
  			...
  			return ' href="'.$link.'" target="'.(!empty($rows[$targ]) ? $rows[$targ] : $blank).'" ';
  		}
  		else {
			$uri  = $uri != "" ? "/".$uri : "";
			$link = $rows[$col] == "" ? $full_url : $full_url."/".trim($link,"/").$uri;
  			if(!empty($rows[$targ]) && $rows[$targ] != ""){
  				return ' href="'.$link.'" target="'.(!empty($rows[$targ]) ? $rows[$targ] : $blank).'" ';
  			}
  			else return ' href="'.$link.'" ';
  		}
	}
	function fullSeoName($rows, $uri = "", $col = "seo_name", $targ = "blank") {
		global $full_url;
		$link = $rows[$col];
		$blank = "";
		if(strstr($link,"http://") != '' ||
	...
	function admin_input_setting_textarea($id){
		if(!empty($_SESSION['admin'])){
			$check = DB_que("SELECT * FROM `#_module_setting` WHERE `id` = '$id' LIMIT 1");
			if(!DB_num($check)) return "";
			$check = DB_arr($check, 1);
			return "<div class='dv-sting'><span>".$check['ten_vi']."</span><textarea onchange='UPDATE_colum(this, \"".$id."\", \"ten_key\", \"#_module_setting\")' class='form-control'>".$check['ten_key']."</textarea><div class='clr'></div></div>";
		}
		return "";
	}
	...
	function f_link301() {
		...
		if($link_check  != "") {
			...
			if(@DB_num($link_check)) {
				$link_check = DB_arr($link_check, 1);
    ...
    $array_dich = array("en" => "en", "cn" => "zh-CN");
	  ...
	  if(empty($_SESSION['admin_lang'])) {
	    $_SESSION['admin_lang'] = "vi";
	  }

	  if(isset($_GET['lang']) && $_GET['lang'] == 'en') {
	    $_SESSION['admin_lang'] = "en";
	  }
	  else if(isset($_GET['lang']) && $_GET['lang'] == 'vi') {
	    $_SESSION['admin_lang'] = "vi";
	  }

	  function glo_lang_admin($text){
	    global $glo_admin;
	    if(empty($glo_admin[$text])) return $text;
	    return $glo_admin[$text];
	  }
    function add_img_admin($opt1, $anh_sp = '', $id = 0, $sql_thongtin = array(), $duongdantin, $hinhanh, $icon = 'icon'){
    	if($opt1 == 0){
			$image_new     = "../".$duongdantin . "/" . $hinhanh;
			$seo_image_new = "../".$duongdantin . "/thumb_" . $hinhanh;

    		if ($anh_sp != '') {
				$anh_sp     = explode("x", $anh_sp);
				$wid        = $anh_sp[0];
				$hig        = $anh_sp[1];
				TAO_anhthumb_new("../" . $duongdantin . "/" . $hinhanh, "../" . $duongdantin . "/thumb_" . $hinhanh, $wid, $hig);
	        }
	        else {
				TAO_anhthumb_new("../" . $duongdantin . "/" . $hinhanh, "../" . $duongdantin . "/thumb_" . $hinhanh, 500, 500);
	        }
    	}
    	else {
	    	if ($anh_sp != '') {
	    ...
	    TAO_anhthumb("../" . $duongdantin . "/" . $hinhanh, "../" . $duongdantin . "/thumbnew_" . $hinhanh, 500, 500);
    ...
    function LAY_bvtn($id_bv){
		$list_data_tn = DB_que("SELECT * FROM `#_baiviet_select_tinhnang` WHERE `showhi` = 1 AND `id_baiviet` = '$id_bv' ORDER BY `id` DESC");
		$list_data_tn = DB_arr($list_data_tn);
		$return = array();
		foreach ($list_data_tn as $rows) {
			array_push($return, $rows['id_val']);
		}
		return $return;
    }

	function fullDate($val, $glo_lang, $the_loai = "")
	{
		$ngaydang = CONVER_thu(date("l", $val), $glo_lang) . ", " . date("H:i", $val) . " " . $glo_lang['date'] . " " . date("d/m/Y", $val);
		if ($the_loai == 1) {
			$ngaydang = date("d/m/Y", $val);
		}
		return $ngaydang;
	}
//	function styleMobile($name)
//	{
//		global $fullpath;
//		$nd = str_replace("../images/","images/",file_get_contents($fullpath.'/css/'.$name.'?=v'.time()));
//		$nd .= str_replace("../delete/","delete/",file_get_contents($fullpath.'/css/'.$name.'?=v'.time()));
//		return $nd;
//	}
?>
